...
// LocalBuildHash — sha256 работающей сборки (коротко) — для диагностики в UI.
// Сравнивается с подписью <архив>.sha256 в релизе, поэтому перезаливка файла
// под тем же номером версии тоже видна как обновление.
func (a *App) LocalBuildHash() string {
	h := update.LocalExeHash()
	if len(h) > 12 {
		return h[:12]
	}
	return h
