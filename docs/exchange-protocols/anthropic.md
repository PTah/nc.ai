...
**Реализовано** (`internal/llm/providers/anthropic`): `ChatCompletion` и `ChatCompletionStream`
(SSE-события `message_start` / `content_block_delta` / `message_delta` / `message_stop`),
...
Credential Manager / Keychain, как у остальных провайдеров.
