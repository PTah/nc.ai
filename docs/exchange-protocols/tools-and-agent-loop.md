...
- **`step`** — milestone: каждые 3 выполненных инструмента (или 25 с, что раньше). Идёт в ленту
  короткой карточкой: «Этап: шаг 4/120 · прошло 1:20 · вызовов инструментов 4 / Сделано: чтение ×3,
  команды ×1 / Сейчас: run_terminal ping 10.0.0.1».
- **`heartbeat`** — каждые 30 с, даже если модель думает. UI не пишет его в чат, а обновляет живую
  строку над полем ввода (таймер, шаг, текущий инструмент, остаток по todo) — появляется после минуты.
