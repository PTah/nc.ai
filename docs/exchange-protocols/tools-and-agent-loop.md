...
| `chat:error` | message |
| `progress` | JSON: phase, step, total, tools, done, current, text |

### 6.1 Этапы работы (`progress`)

Агент сам рассказывает, на каком этапе находится — рассуждения при этом остаются свёрнутыми:

- **`step`** — milestone: каждые 3 выполненных инструмента (или 25 с, что раньше). Попадает в ленту
  короткой карточкой «Этап: шаг 4/120 · прошло 1:20 · вызовов инструментов 4 / Сделано: чтение ×3,
  команды ×1 / Сейчас: run_terminal ping 10.0.0.1» **только если** запрос просил промежуточные
  результаты либо агент молчит без вывода дольше двух минут. Иначе — тишина.
- **`heartbeat`** — каждые 30 с, даже если модель думает. UI не пишет его в чат, а обновляет живую
  строку над полем ввода (таймер, шаг, текущий инструмент, остаток по todo).
- **`start`** — старт задачи; в чат не попадает.
- **`notice`** — предупреждение о длинном прогоне: по шагам (`AgentMaxSteps < 0` = без предела,
  `AgentWarnSteps` — порог). Показывается в чате системной строкой.

Ручной запрос «показывай промежуточные результаты» поддержан в system prompt: агент пишет короткий
статус этапа обычным текстом после каждого блока работы.
