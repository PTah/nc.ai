...
В Settings → Local есть поле **Context** — оно не отправляется в Ollama (см. выше), но по нему приложение считает заполнение: если промпт подошёл к 85 % окна, в чат уходит предупреждение. Для llama.cpp-совместимых серверов значение уходит как `options.num_ctx`.
