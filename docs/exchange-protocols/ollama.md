...
В Settings → Local есть поле **Context** — оно не отправляется в Ollama (см. выше), но по нему приложение считает заполнение: в статус-баре видно `ctx 12.4k / 32k`, а когда промпт подошёл к 85 % окна, в чат приходит предупреждение (если окно не задано — предупреждение срабатывает на 8k). Для llama.cpp-совместимых серверов значение уходит как `options.num_ctx`.
